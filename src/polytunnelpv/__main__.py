...
class OperatingMode(enum.Enum):
    ...
    HOURLY_MPP_INTERPOLATION: str = "hourly_mpp_interpolation"
...
def process_single_mpp_calculation_without_pbar_interpolation(
    time_of_day: int,
    *,
    irradiance_frame: pd.DataFrame,
    locations_to_weather_and_solar_map: dict[
    ...
    try:
        ...
        for pv_cell in tqdm(
            ...
            (
                ...
                voltage_series,
                pv_cells_bypassed,
            ) = pv_cell.calculate_iv_curve_interpolation(
...
def create_voltage_interpolating_array(
    *,
    ...
    voltage_series = Parallel(n_jobs=8)(delayed(
                        modified_bishop_voltage_wrapper_function
                        )(
                        current,
                    ...
                    for current in (
                        tqdm(current_series, desc='Calculating Voltage Array')  # type: ignore [union-attr]
                        )
                    )
    ...
    for i,volt in enumerate(voltage_series):
        if isinstance(volt,int):
            ...
            ravelled_voltage = volt.ravel()
            for index,v in enumerate(ravelled_voltage):
                if v > (
                    module_breakdown_voltage := (
                        pv_cell_used.num_cells_in_parent_module * pv_cell_used.breakdown_voltage
                    )
                ):
            ...
            voltage_series[i] = ravelled_voltage.reshape(series_shape)
...
def main(unparsed_arguments) -> None:
    ...
    match operating_mode.value:
        ...
        case OperatingMode.VALIDATION.value:
            ...
            # Plot the validation days for EUPVSEC
            import pdb
            ...
            sns.despine(ax=ax4, right=True, left=True)
            ...
            sns.despine(ax=ax6, right=False, left=True)
            ...
            sns.despine(ax=ax8, right=True, left=True)
            ...
            sns.despine(ax=ax10, right=True, left=True)
            ...
            ax1.set_xlim(6, 18)
            ax2.set_ylim(0, 1)
            ax1.legend(loc="upper left", title="Cloudy")
            # ax2.legend(loc="upper right")
            ax1.tick_params(right=False)
            ax2.tick_params(left=False, right=False)
            ax2.set_yticklabels([])
            ...
            ax5.set_xlim(6, 18)
            ax6.set_ylim(0, 1)
            ax5.legend(loc="upper left", title="Partly cloudy")
            # ax2.legend(loc="upper right")
            ax5.tick_params(right=False)
            ax6.tick_params(left=False, right=False)
            ax6.set_yticklabels([])
            ...
            ax9.set_xlim(6, 18)
            ax10.set_ylim(0, 1)
            ax9.legend(loc="upper left", title="Sunny")
            # ax3.legend(loc="upper left")
